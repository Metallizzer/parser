...
class MosconlineRuParser implements ParserInterface
{
    use \app\components\helper\metallizzer\Cacheable;

    const USER_ID  = 2;
    const FEED_ID  = 2;
    const SITE_URL = 'https://mosconline.ru';

    protected static $posts = [];

    public static function run(): array
    {
        $url  = self::SITE_URL.'/archive.php';
        ...
        $page->filter('.portfolio_container .col-md-2')->each(function ($item) {
            self::loadPost($item);
        });
    ...
    {
        $url = Url::encode($item->filter('a')->link()->getUri());
        ...
        $post = new NewsPost(
            self::class,
            html_entity_decode($item->filter('a span')->last()->text()),
            '~',
        ...
        self::$posts[] = (new Parser())->fill(
            $post,
            $page->filterXpath('//div[@class = "col-md-9"]/p[1]/node()')
        );
    }
